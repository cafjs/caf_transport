...
(function () {
      ...
      * @param {caf.msg} msg
      * @param {Object} target
      * @param {caf.cb} cb Returns error of type {msg:caf.msg}, where the msg
      * field is a system error message, or the result of the method invocation.
      *
      *
      * @function
      */
     json_rpc.call = function(msg, target, cb) {
         ...
         var wrapError = function(errMsg) {
             var err = new Error(errMsg.error.msg);
             err.msg = errMsg;
             return err;
         };
         if (typeof target !== 'object') {
             error = wrapError(systemError(msg, ERROR_CODES.noSuchCA,
                                           'CA not found'));
         }
         if ((!error) && !(isRequest(msg) || isNotification(msg))) {
             error = wrapError(systemError(msg, ERROR_CODES.invalidRequest,
                                           'Invalid request'));
         }
         if ((!error) && (typeof target[msg.method] !== 'function')) {
             error = wrapError(systemError(msg, ERROR_CODES.methodNotFound,
                                           'method not found'));
         }
         if (!error) {
             try {
                 ...
                 error = wrapError(systemError(msg, ERROR_CODES.invalidParams,
