...
(function () {
     ...
     json_rpc.systemRequest = function(to, methodName) {
         return json_rpc.request(json_rpc.SYSTEM_TOKEN, to,
     ...
     var systemError = json_rpc.systemError = function(request, code, errMsg,
         ...
         var error = {
             'code' : code,
             'message' : errMsg,
             'data' : [newReplyMeta(request), toErrorObject(err)]
         };
         return {
             'jsonrpc': '2.0',
             'error' : error,
      ...
      * caf.sysError {sysErrMsg: caf.msg}, where the msg
      * field is a system error message, or an application error,
      *  or, in the second argument, the result of the method invocation.
      *
      *
      * @function
      */
     json_rpc.call = function(msg, target, cb) {
         ...
         if (typeof target !== 'object') {
             error = systemError(msg, ERROR_CODES.noSuchCA,
                                 'CA not found');
         }
